...
客户端连接成功后，需要在 Web 管理界面创建隧道，将内网服务暴露到公网。

### 步骤 1：确认客户端在线

1. 登录 Web 管理界面 (`http://你的服务器IP:8080`)
2. 左侧「客户端」查看你的客户端是否显示「在线」
3. 如果离线，检查客户端日志 `tail -f npc.log`

### 步骤 2：创建 TCP 隧道

假设你要将本地 MacBook 上的服务（端口 8080）暴露到公网：

1. 左侧菜单「TCP 隧道」→「新增」
2. 填写配置：
   - **类型**：TCP
   - **客户端**：选择你的客户端（如 "MacBook"）
   - **服务端端口**：`9001`（外网访问时用此端口）
   - **目标 (IP:端口)**：`127.0.0.1:8080`（本地服务的地址和端口）
   - **备注**：本地开发环境（可选）
3. 点击「新增」
...
| 场景 | 本地服务 | 目标地址 | 服务端端口 | 访问地址 |
|------|---------|---------|-----------|---------|
| 本地网站 | 8080 | `127.0.0.1:8080` | 9001 | `http://服务器IP:9001` |
| Home Assistant | 8123 | `127.0.0.1:8123` | 9002 | `http://服务器IP:9002` |
| NAS 管理界面 | 5000 | `127.0.0.1:5000` | 9003 | `http://服务器IP:9003` |
| SSH 远程连接 | 22 | `127.0.0.1:22` | 9004 | `ssh 用户名@服务器IP -p 9004` |

### 创建多个隧道

可以为一个客户端创建多个隧道：
- 隧道 1：`9001` → `127.0.0.1:8080`（网站）
- 隧道 2：`9002` → `127.0.0.1:8123`（Home Assistant）
- 隧道 3：`9003` → `127.0.0.1:3306`（数据库，仅限私密访问）

**注意**：服务端端口必须在 `.env` 配置的 `NPS_ALLOW_PORTS_START` 和 `NPS_ALLOW_PORTS_END` 范围内（默认 9000-9100）。
...
client id xxx connection succeeded
```

### 方法三：查看连接数统计

在 Web 管理界面「概览」页面，可以看到：
- 在线客户端数量
- 当前连接数
- 流量统计
...
假设客户端创建了一个 TCP 隧道（服务端端口 9001 → 本地端口 8080）：
